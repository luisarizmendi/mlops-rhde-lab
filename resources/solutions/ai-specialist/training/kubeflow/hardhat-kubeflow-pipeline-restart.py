...
def train_model(
    ...
    # Set up training directory and check for last checkpoint
    train_dir = f"runs/train/{CONFIG['name']}"
    checkpoint_path = os.path.join(train_dir, "weights", "last.pt")

    if os.path.exists(checkpoint_path):
        print(f"Resuming training from {checkpoint_path}")
        model = YOLO(checkpoint_path)  # Load last checkpoint
        resume_training = True
    else:
        print("Starting training from scratch")
        model = YOLO(CONFIG['model'])  # Load base model
...
def yolo_training_pipeline_restart(
...
if __name__ == "__main__":
    ...
    compiler.Compiler().compile(
        pipeline_func=yolo_training_pipeline_restart,
        package_path='yolo_training_pipeline_restart.yaml'
    )
    print("Pipeline compiled successfully to yolo_training_pipeline_restart.yaml")
