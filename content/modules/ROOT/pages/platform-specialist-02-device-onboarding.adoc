...
Make sure you have installed `oc` and `helm` cli on your own Workstation. You can find them here {openshift-console}/command-line-tools

<<<<<<< HEAD
Login to the shared lab OpenShift {openshift-console}[cluster] using the provided username and password {openshift-user-base}pass:[<span id="gnumberVal"></span>]:{openshift-password-base}pass:[<span id="gnumberVal"></span>]. 
=======
Login to the shared lab OpenShift {openshift-console}[cluster] using the provided username and password {openshift-user-base}:{openshift-password-base} from your Workstation and proceed with installation. 
>>>>>>> 68716c399ad6fec56da4a7eb663eab19bda068cb
...
$ helm upgrade --install --version=0.4.0 \
    --namespace user<span id="gnumberVal"></span> --create-namespace \
