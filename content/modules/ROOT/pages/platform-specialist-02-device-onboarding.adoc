...
Login to the shared lab OpenShift https://console-openshift-console.apps.{ocp_cluster_url}[cluster] using the provided username and password {openshift-user-base}pass:[<span id="gnumberVal"></span>]:{openshift-password-base}pass:[<span id="gnumberVal"></span>] from your Workstation. 
...
----
$ oc new-project user<span id="gnumberVal"></span>
$ helm upgrade --install --version=0.4.0 \
    --namespace user<span id="gnumberVal"></span> \
...
[source,shell,role=execute,subs="attributes"]
----
$ oc get pods -n user<span id="gnumberVal"></span>
...
[source,shell,role=execute,subs="attributes"]
----
$ oc project user<span id="gnumberVal"></span>
...
[source,bash]
----
$ flightctl login https://api.user<span id="gnumberVal"></span>.apps.{ocp_cluster_url} --web --insecure-skip-tls-verify
...
[NOTE]
====
Should you not have any DISPLAY environment specified on your machine, you can also login just using CLI on terminal

[source,bash]
----
$ flightctl login https://api.user<span id="gnumberVal"></span>.apps.{ocp_cluster_url} --insecure-skip-tls-verify --username {flightctl-user-basename} --password {flightctl-password}
----
====
...
With this file available we are now ready to build the new Containerfile (_either on the Shared Edge Builder or on your Local Edge Device using KVM_)
...
pushing the new image to the Edge Container Registry


[source,shell,role=execute,subs="attributes"]
----
$ sudo podman push <image-id> {container-registry-gitea}/<span id="gnumberVal"></span>/nvidia:0.0.2
----
