...
Username and password for the Shared Edge Builder are {shared-nvidia-user}:{shared-nvidia-pass}. 
...
Log into the Shared Edge Builder at {shared-nvidia-ip} with ssh credentials {shared-nvidia-user} : {shared-nvidia-pass}

Make sure to use the folder with your "userpass:[<span id="gnumberVal"></span>]" on the shared Shared Edge Builder and copy the created Containerfile there.
...
Grab the baked ISO with `scp` from the Shared Edge Builder to your Workstation and burn the ISO to a USB stick and get ready to boot your assigned Local Edge Device.
