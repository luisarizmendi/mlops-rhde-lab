...
We will use as starting point a prebuilt bootc-compatible image and we will build on it, with the objective to include a way to allow us to reach the Nvidia Device remotely, without plugging on to the serial console or visualizing it through the DisplayPort.
...
NOTE: Make sure to pull the container image with `sudo` since the last step of building the bootc image requires `sudo` permissions.
...
For the third point, we are going to use the following script and add that as a service in our Containerfile so that it is started at boot time (`GROUP-NAME-CHANGE-ME` in your case is "userpass:[<span id="gnumberVal"></span>]")
...
NOTE: To build this first new image you are going to use the Edge Nvidia AGX Device available on this network at the following address {shared-nvidia-ip} since we are assuming that you don't know yet how to reach the Nvidia Device you have in front of you (and you can build images using bootc only on same as target device architecture devices). 
Username and password for the shared Nvidia Device are {shared-nvidia-user}:{shared-nvidia-pass}. 

WARNING: Watch out for using your group assigned Nvidia Device to try and build the following iteration of the Device Image! To do that you would need a fully subscribed RHEL 9 System, with the same system architecture as the target device (so *aarch64*). + 
So for this step you would need to use the shared Edge Nvidia Device mentioned above and if you want to build the rest of the images in the track on your group assigned Nvidia Device, I suggest you include Virtualization in the Containerfile. +
In that case you would create a full RHEL9 VM, running on KVM, that _bakes_ new images. +
...
You can find the complete Containerfile here (`GROUP-NAME-CHANGE` in your case is "userpass:[<span id="gnumberVal"></span>]"):
...
JSON="{ \
\"body\":\"\$IP_ADDRESS\", \
\"title\": \"pass:[<span id="gnumberVal"></span>]\" \
}"
...
Make sure to use the folder with your "userpass:[<span id="gnumberVal"></span>]" on the shared Edge Nvidia Device and copy the created Containerfile there.  
...
[source,bash,subs="attributes"]
----
$ sudo podman push <imageID> {container-registry-gitea}/GROUP-NAME-CHANGE-ME/nvidia:0.0.2
----


== Image Deploy

Now that you have built the "refreshed" container image we need to deploy it to your own Nvidia Device sitting on your desk.
...
[[registry]]
location = "{container-registry-gitea}"
insecure = true
EOF
bootc switch --mutate-in-place --transport registry {container-registry-gitea}/GROUP-NAME-CHANGE-ME/nvidia:0.0.2
%end
...
Now we can go ahead and create the ISO image (make sure to save the above file as config.toml and create an output directory).

TIP: we introduce the parameter `--tls-verify=false` as we are using an insecure registry.

[source,bash,subs="attributes"]
----
$ sudo podman run     --rm     -it     --privileged    --pull=newer     --security-opt label=type:unconfined_t     -v /var/lib/containers/storage:/var/lib/containers/storage     -v $(pwd)/config.toml:/config.toml     -v $(pwd)/output:/output     registry.redhat.io/rhel9/bootc-image-builder:latest     --type iso  --tls-verify=false    --config /config.toml   {container-registry-gitea}/GROUP-NAME-CHANGE-ME/nvidia:0.0.2
----

NOTE: during the build process the `nvidia-local` repo will fail, it is normal since it is a local only repo, created with the initial image (0.0.1). The whole building process will last approximately 15 minutes. 


== Image Rollout

You can now burn the ISO to a USB stick and boot your assigned Nvidia Device.
...
The automated installation will take approximately 10 minutes after which the Device will reboot and post the detected IP address on Gitea, inside the `inventories` repo, as an issue: 

image::platform-image-mode-gitea-inventories.png[]

You will now have access to the assigned Nvidia Device with one of the defined users (either in the Containerfile or Kickstart file) and could also access the Cockpit interface of the same.

You can now also verify that the Device is pointing to the Edge Container Registry by running the following: 
...
NOTE: You will see healthchecks for Microshift fail, since we have yet to include the pull-secret needed to pull container images into the Device. We can go ahead and disable Microshift for now with+ 
[source,bash]
----
