...
You would then need to log into the Edge Container Registry with the credentials {container-registry-gitea-user} : {container-registry-gitea-pass}
...
insecure = true
EOF
bootc switch --mutate-in-place --transport registry {container-registry-gitea}/YOUR-GROUP-NUMBER/nvidia:0.0.2
%end
