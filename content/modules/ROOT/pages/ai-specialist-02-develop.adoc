...
== (optional) Create a custom Container Image

You can create a custom Container base image, so for example you can avoid having to install the dependancies each time that you create an environment for the Jupyter Notebook.

The first step is to create a Containerfile prepared to run in OpenShift. In our case we need to start from a base image that is prepared for pytorch and install the ultralytics, roboflow and minio dependancies. Something like this one:
...
Then build your Container Image and push it into a registry:
...
If you don't want to build the image you can use this one: quay.io/luisarizmendi/pytorch-custom:latest
...
=== Create the step Jupyter Notebooks
...
ADD VOLUME 
* create PVC  (training-elyra-pipeline with 1G is enough in the userxx-ai project)
