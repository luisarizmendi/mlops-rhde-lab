...
== (optional) Prototyping Pipeline

So far, you've used a Jupyter Notebook for quick model prototyping. While this approach is excellent for experimentation, transitioning to a more structured and scalable workflow can bring numerous benefits, such as ensuring consistency, repeatability, and improved collaboration.
...
1. navigate to "Data Science Pipelines" and add a new server
2. Import the Data Connection data but change the bucket to "userpass:[<span id="gnumberVal"></span>]-ai-pipelines" and Region `none` (it is not configured in Minio)
3. Save
4. Restart the workbenchs that are running
...
Once you have added your custom container image to the Jupyter environment, you can double-check if the Pipeline Server has been correctly loaded. Open the Jupyter Notebook workbench and navigate to the "Runtimes" section by clicking the gear icon in the left menu. Confirm that the runtime configuration has been automatically loaded. If everything looks correct, your environment is ready for pipeline execution.
