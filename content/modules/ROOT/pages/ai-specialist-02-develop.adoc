...
3- Create a new S3 Storage Connection that will be used by your Jupyter Notebooks to save the model and performance stats. Use your MinIO username and password ( Access key={minio-user-base}pass:[<span id="gnumberVal"></span>] / Secret key={minio-password-base}pass:[<span id="gnumberVal"></span>] ), the MinIO API URL ({minio-api}) (removing `http://`) and the Bucket name "userpass:[<span id="gnumberVal"></span>]-ai-models" (you can keep the Region empty). 

image::ai-build-dataconnection.png[]

4- Create a new Workbench named "Object Detection Notebooks". You will need to select the base image that will be used to run your Jupyter Notebooks (select `PyTorch`), the Container Size (`Small` is enough), the Persistent Volume associated to the container (you can keep the default 20Gi Persistent Volume for your Notebook but you won't need that much storage) and configure the Object Storage Connection that you already configured. One additional thing to take into account is that when you have GPUs and you have defined `Accelerator profiles` in your environment (`Settings > Accelerator profiles`) you will find that during the Workbench creation you don't only can select the instance size, but also if you want to use accelerators (see an example below with NVIDIA GPUs).
...
In order to make it easy, we will use the created Jupyter Notebook that we used to quick prototyping the model, but we cannot use it directly, for example since in a pipeline each step is run independently you will need to include a way to pass the files and the variables from one step into the other.

[NOTE]

This exercice is also useful to be better prepared to implement the training pipeline based on Kubeflow that we will implement in the xref:ai-specialist-03-training.adoc[Model Training] section.

Let's review the changes that you need to introduce:
...
Rename .pipeline file "training-elyra.pipeline"
...
ADD VOLUME 
* create PVC  (training-elyra-pipeline with 5G is enough in the userxx-ai project)
