...
2. Click "Sign In" (top right button) with user "{gitea-user-base}pass:[<span id="gnumberVal"></span>]" and password "{gitea-password-base}pass:[<span id="gnumberVal"></span>]"
...
1. Navegate to https://minio-ui-minio.apps.{ocp_cluster_url}
...
** MinIO API URL: https://minio-api-minio.apps.{ocp_cluster_url}
...
8- Clone the source code repository that you created ("userpass:[<span id="gnumberVal"></span>]-ai") using the left menu (you can find the repository clone URL opening the Gitea repository). Once you click "Clone" a message will appear in the button right. It could take some time to clone the repository.

image::ai-build-gitclone.png[]

9- Create a `prototype.ipynb` file inside the cloned directory ("userpass:[<span id="gnumberVal"></span>]-ai")
====
...
You now can go to MinIO console ( https://minio-ui-minio.apps.{ocp_cluster_url} ) and open your "userpass:[<span id="gnumberVal"></span>]-ai-models" Bucket. You will find the model with the best performance metrics (`best.pt`) and the last produced with the last training epoch (`last.pt`) along with the performance stats for the Test, Validation and Training sets.
