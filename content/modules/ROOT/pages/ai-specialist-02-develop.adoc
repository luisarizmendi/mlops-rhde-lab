...
You now can go to MinIO console ({minio-ui}) and "Browse Files" on your "userpass:[<span id="gnumberVal"></span>]-ai-models" Bucket.
...
[NOTE]
====
If you are running in an environment with GPUs, you must specify the GPU number and GPU Vendor (`nvidia.com/gpu`). GPUs will be used during training, but you must include the GPU tag in all tasks. Alternatively, you can configure it in the Pipeline Properties to apply it globally.

This is necessary for Kubernetes to schedule the pod on a GPU-enabled node. Since tasks share a Persistent Volume (PV), there is a risk that GPU nodes may be in a different zone where the PV cannot be shared with non-GPU nodes. If this happens, Kubernetes will always select a node without GPUs, or you may encounter an error if scheduling is forced.
