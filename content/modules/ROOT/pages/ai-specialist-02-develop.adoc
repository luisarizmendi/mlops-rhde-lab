...
4. Click the square with an arrow inside on the top right corner. A new menu on the right will appear with different tabs to configure aspects of the pipeline and the different steps (if you click a step the step configuration will be opened). You need to configure the following:
...
** In each step you will need to fill in the required Environment Variable values.
...
The Persistent volume is not autogenerated, so you will need to create the Persistent Volume Claim manually with the specified name in the project where you will be running the pipeline ("userpass:[<span id="gnumberVal"></span>]-ai")
