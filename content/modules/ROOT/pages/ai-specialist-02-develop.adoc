...
You now can go to MinIO console ( {minio-ui} ) and open your "userpass:[<span id="gnumberVal"></span>]-ai-models" Bucket. You will find the model with the best performance metrics (`best.pt`) and the last produced with the last training epoch (`last.pt`) along with the performance stats for the Test, Validation and Training sets.
...
1. Navegating to OpenShift AI: {openshift-ai}
