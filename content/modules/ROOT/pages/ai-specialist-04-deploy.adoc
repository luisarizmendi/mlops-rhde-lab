...
For the Inference Server you will find two different Containerfiles, one can be used to create a container image that runs on your laptop and the other one is building the container image that runs on the NVIDIA Jetson devices. You will be using the later during the Platform Specialist module of this workshop.
