...
1- Navegate to {openshift-ai}[OpenShift AI]
...
9- Move into the `workshop-object-detection-rhde/resources/solutions/ai-specialist/development` directory. Open the `prototyping.ipynb`
 file
...
3- Create a new OpenShift Project (`userpass:[<span id="gnumberVal"></span>]-ai-test`)
...
When all pods are running, you will be able to open the Dashboard using the Frontend URL. The Dashboard application does not use TLS, so the URL must start `http://` and `https://` otherwhile you will get a message "Application is not available" even when then POD is already running.
