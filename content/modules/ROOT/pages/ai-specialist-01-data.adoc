...
4. Go to btn:[Projects] on the left menu and click btn:[+New Project]. Give it a name (ie. "group-pass:[<span id="gnumberVal"></span>]-hardhat-detection"), include an annotation group (ie. "hardhats") and select "Object Detection" as `Project Type`.
====
...
3. Identify one or multiple datasets with relevant labeled data by playing with the btn:[Advanced Filters]. You can add `class:<name>` into the search box to only show datasets that contains data with the 'name' label, for example `class:hardhat`.
====
...
[example]
====
To accurately determine the number of relevant images in a specific dataset, follow these steps:

1. Navigate to the dataset's URL
2. Click the btn:[Images] button
...
Select just one class at a time, since if you select multiple classes an "AND" operator will be used so only images with both classes will appear in the selection.
...
Now you need to create your own Dataset out of the labeled images of the source Dataset/s. In order to do that you have two options: you can fork an entire Dataset in your account, or you can clone only the images with the interesting labels. Cloning the images with the required labels makes more sense than forking the entire Dataset, including images that are not relevant to our use case.
...
5. Check that all images are selected and then click btn:[Clone <number> Selected Images] on the top right corner. Select the Workspace and the Project that you created before and click btn:[Clone <number> Images]

[NOTE]
If the page does not respond or you find errors, try to clone images in batches instead of performing a single clone with a high number of items.
====
...
[TIP]
====
If you find any issue while cloning images or if you don't want to expend time doing this work, you can just *Fork* the following Dataset (you will find less issues forking than cloning in Roboflow, even if an error is shown the images are copied into your account):

https://universe.roboflow.com/luisarizmendi/hardhat-or-hat

After cloning be sure that you remove images with other classes than `hardhat` or `no_hardhat`. In order to do that you have to go to "Dataset" in the left manu, select the class that you want to delete, perform the trick to show all images and select them all by pressing the box, and choose the "Remove from Project" action
