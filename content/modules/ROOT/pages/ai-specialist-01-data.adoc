...
Roboflow includes some restrictions to Not-Paid accounts. The most important for us is that the overall number of images in your account must be less than 10000, so we need to mantain the size of the Dataset that we will create in the next point below that number. 
...
Roboflow Universe offers many datasets created from scratch by people  (but also hundreds of copy-pasted ones), so how to select the right datasets for our propuse? Simplifying a lot, in our case you need to look for the right labels and the "right" number of images.

The "right" number of images is always as much as you can, since the AI model performance is most of the time directly proportional to the number of available data.

[CAUTION]

In our case, since we are using a Roboflow not-paid account, we cannot go beyond 10000 per dataset.
...
.Looking for Datasets in Roboflow Universe
[example]
====
...
.Finding the right pre-created datasets
