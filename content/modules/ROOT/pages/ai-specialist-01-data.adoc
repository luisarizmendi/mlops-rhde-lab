...
4. (Optional) Go to "Projects" on the left menu and click "+New Project". Give it a name (ie. "user-pass:[<span id="gnumberVal"></span>]-hardhat-detection"), include an annotation group (ie. "hardhats") and select "Object Detection" as `Project Type`. This step is optional because if you fork a Dataset (explain later) it will create a new Project for you. 
====
...
project = rf.workspace("workspace").project("user-pass:[<span id="gnumberVal"></span>]-hardhat-detection")
...
roboflow:
  ...
  project: user-99-hardhat-detection
  version: 1
  license: CC BY 4.0
  url: https://app.roboflow.com/workshop-igjqz/user-99-hardhat-detection/1
----
...
Now you’re ready to proceed to the xref:ai-specialist-02-build.adoc[Model Building] section. However, it’s important to keep in mind that model training is a time-intensive process. Depending on your hardware and hyperparameter configuration, training can take anywhere from several hours to multiple days. If your goal is to complete this workshop in a single session, you’ll need to consider an alternative approach: the "Plan B."

Plan B for model training during this workshop involves significantly reducing the dataset size to shorten the training time. While this means the resulting model won’t be viable for real-world use, it will allow you to complete all the steps in the training phase within the limited timeframe.

[NOTE]
A pre-trained model will be provided as the solution in the xref:ai-specialist-02-build.adoc[Model Building] section, which you can use to complete the subsequent steps.
