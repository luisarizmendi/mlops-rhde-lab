...
* Data Preparation: Cleaning, augmenting, splitting and labeling data to ensure it's ready for training the AI model.
...
In our case, Roboflow offers a feature to perform data augmentation when creating your Dataset from the images in your Project. Since the Dataset split is also handled during the same step, we will cover data augmentation in the following section as well.
